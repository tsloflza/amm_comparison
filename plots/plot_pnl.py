...
def plot_hedged_pnl(sim_result: dict, amm_name: str,
    ...
    Shows:
      ...
      - hedged_pnl at each frequency      — coloured lines
      - fees_minus_lvr (theoretical)      — dashed black
    ...
    Parameters
    ----------
    sim_result : output dict from engine.run_simulation()
    """
    prices  = sim_result["prices"]                  # (n_steps+1, n_paths)
    ...
    # Pool P&L cumulative: V(P_t) - V(P_0) path-by-path, then average
    # We approximate using the final scalar pool_pnl (single scalar per path)
    # For a time-series we reconstruct from the prices array using the AMM
    # (we skip this to keep the plot module decoupled; instead show distribution)

    pool_pnl_mean  = float(np.mean(sim_result["pool_pnl"])) * 100
    pool_pnl_std   = float(np.std(sim_result["pool_pnl"]))  * 100

    ax1.bar([0], [pool_pnl_mean], color="grey", alpha=0.5, label="pool_pnl (mean)", width=0.4)
    ...
    ax1.set_title(f"Fig 6 – Hedged vs Unhedged LP Returns\n{amm_name}", fontsize=10, fontweight="bold")
    ax1.legend(fontsize=8, framealpha=0.9)
    ax1.grid(True, axis="y", alpha=0.3)
    x_labels = ["pool_pnl"] + [FREQ_LABELS[f] for f in sorted(sim_result["hedged_pnl"])] + ["fees−LVR"]
...
def plot_sharpe_vs_rebal(sim_results_by_amm: dict, T_days: int = 30,
    ...
    Parameters
    ----------
    sim_results_by_amm : dict  amm_name → run_simulation output
                         (all with same sigma and volume scenario)
    """
    ...
    ax.set_title("Fig 7 – Sharpe Ratio vs Delta-Hedge Frequency", fontsize=11, fontweight="bold")
...
def plot_radar(amms: list, sigma: float = 0.05, tvl_frac_for_slip: float = 0.01,
    ...
    Spider/radar chart comparing AMMs across 5 dimensions:
      1. Low Slippage        (1 - slippage_1pct, rescaled so higher = better)
      2. Low IL              (1 - IL_at_20pct_move)
      3. Low LVR/TVL         (1 - normalised LVR)
      4. Capital Efficiency  (normalised marginal liquidity)
      5. Fee Income Potential (fee_tier, normalised)

    All metrics are linearly rescaled to [0, 1].
    """
    ...
    for amm in amms:
        ...
        lvr   = daily_lvr_analytical(amm, sigma) * 100 * 365
        ml    = amm.marginal_liquidity(amm.P0) * amm.P0   # dimensionless sensitivity
        fee   = amm.fee_tier * 100
        raw[amm.name] = [slip, il20, lvr, ml, fee]

    # Normalise each metric to [0,1] across the AMMs, then invert "bad" metrics
    ...
    norm = (raw_arr - col_min) / col_rng       # [0,1], higher = larger raw value

    # Invert metrics where lower is better (slippage, IL, LVR, fee)
    # Capital efficiency: higher is better → keep as is
    for col in [0, 1, 2, 4]:               # slippage, IL, LVR, fee
    ...
    for i, (amm, values) in enumerate(zip(amms, norm)):
    ...
    ax.set_title("Fig 11 – AMM Comparison Radar\n(higher = better on each axis)",
