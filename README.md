...
$$x^{\ast}(P) = L\left(\frac{1}{\sqrt{P}} - \frac{1}{\sqrt{P_b}}\right), \quad y^{\ast}(P) = L\left(\sqrt{P} - \sqrt{P_a}\right)$$
...
$$\text{LVH}_t = R^{\text{HODL}}_t - V_t = (P_t x^{\ast}(P_0) + y^{\ast}(P_0)) - V(P_t)$$
